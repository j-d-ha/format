...
- Keep each case focused: inputs, expected outputs, expected errors, and any setup data belong in
  the struct.
...
- For complex values, use `cmp.Diff`, `reflect.DeepEqual`, or equivalent project conventions rather
  than hand-written field comparisons.
- Keep test helpers small, mark them with `t.Helper()`, and avoid sharing mutable state between
  table cases.
